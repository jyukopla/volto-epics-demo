...
let
  ...
  overrides = self: super: {
    "datamodel-code-generator" = super."datamodel-code-generator".overridePythonAttrs(old: {
      postInstall = ''
        rm -r $out/${buildPython.sitePackages}/tests
      '';
    });
    "pydantic" = super."pydantic".overridePythonAttrs(old: {
      patches = [];
    });
    "random2" = super."random2".overridePythonAttrs(old: {
      doCheck = false;
    });
    "httpx" = super."httpx".overridePythonAttrs(old: {
      propagatedBuildInputs = old.propagatedBuildInputs ++ [ self."idna" ];
    });
    "ujson" = super."ujson".overridePythonAttrs(old: {
      nativeBuildInputs = [ self."setuptools_scm" ];
    });
  };
...
#         ("python" + replaceStrings ["."] [""] pythonPackages.python.pythonVersion)
  ...
  appPython = targetPython.withPackages(ps: [ ps.uvicorn package ]);
  ...
  build = stdenv.mkDerivation {
    ...
    installPhase = ''
      ...
      cat > $out/bin/${bin}<< EOF
      #!/usr/bin/env sh
      ${appPython}/bin/python -m uvicorn.main --proxy-headers app.main:app \$@
      EOF
      chmod u+x $out/bin/${bin}
    '';
    buildInput = [ appPython ];
  };
